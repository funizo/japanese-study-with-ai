...
const categoryColors: Record<string, string> = {
  동사: "border-pink-500/40 text-pink-400 bg-pink-500/10",
  명사: "border-violet-500/40 text-violet-400 bg-violet-500/10",
  형용사: "border-blue-500/40 text-blue-400 bg-blue-500/10",
  부사: "border-amber-500/40 text-amber-400 bg-amber-500/10",
  표현: "border-teal-500/40 text-teal-400 bg-teal-500/10",
};
...
export default function VocabularyPage() {
  ...
  const search = async (w?: string) => {
    ...
    try {
      ...
      setResult({ raw: "❌ 오류가 발생했습니다. 다시 시도해주세요." });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div style={{ minHeight: "100vh", width: "100%" }}>
      ...
      >
        ...
        >
          ...
          <h1 className="font-bold text-white text-sm">📚 단어 학습</h1>
        </div>
      </header>
      ...
      >
        ...
        <div className="glass rounded-2xl border border-white/10 p-5 mb-5">
          ...
          <div className="flex gap-2 mb-4">
            ...
            <button
              ...
              className="px-5 py-4 bg-gradient-to-r from-pink-500 to-rose-600 rounded-xl text-white font-bold hover:shadow-lg transition-all duration-200 hover:scale-105 disabled:opacity-40 disabled:cursor-not-allowed flex-shrink-0 text-sm"
            >
              검색
            </button>
          </div>
        ...
        {/* AI 추천 단어 섹션 */}
        <div className="glass rounded-2xl border border-violet-500/30 p-5 mb-5 bg-gradient-to-br from-violet-500/5 to-purple-600/5">
          ...
          {recommendations.length === 0 && !recLoading && (
            <button
              onClick={fetchRecommendations}
              className="w-full py-4 rounded-xl border border-dashed border-violet-500/40 text-violet-400 text-sm font-semibold hover:bg-violet-500/10 hover:border-violet-500/70 transition-all duration-200 flex items-center justify-center gap-2"
            >
              <span>✨</span> AI 추천 단어 보기
            </button>
          )}
          {recLoading && (
          ...
          {!recLoading && recommendations.length > 0 && (
            <div className="grid grid-cols-1 gap-2">
              {recommendations.map((rec, i) => (
                ...
                >
                  <div className="w-6 h-6 rounded-full bg-violet-500/20 flex items-center justify-center text-violet-400 text-xs font-bold flex-shrink-0">
                    {i + 1}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-baseline gap-2">
                      <span className="text-base font-bold text-white font-jp group-hover:text-violet-300 transition-colors">
                        {rec.word}
                      </span>
                      <span className="text-xs text-white/40 font-jp">
                        {rec.reading}
                      </span>
                    </div>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full border ${categoryColors[rec.category] || "border-white/20 text-white/40"}`}
                    >
                      {rec.category}
                    </span>
          ...
          {!recLoading && recAttempted && recommendations.length === 0 && (
            <p className="text-white/30 text-sm text-center py-2">
              추천 단어를 불러오지 못했습니다 — 다시 시도해보세요
            </p>
          )}
        </div>
        ...
        {result && !loading && (
          <div className="glass rounded-2xl border border-violet-500/30 p-5 animate-fade-in-up bg-gradient-to-br from-violet-500/10 to-purple-600/10">
                ...
                <button
                  ...
                  className={`ml-auto flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold transition-all duration-200 ${
                    saved
                      ...
                      : "glass border border-violet-500/40 text-violet-400 hover:border-violet-400 hover:bg-violet-500/10"
                  }`}
                >
            ...
            <div>{renderResult(result.raw)}</div>
          </div>
        )}
